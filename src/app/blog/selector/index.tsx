...
export const Selector: FC<ISelector> = ({ className }) => {
  ...
  useGSAP(() => {
    const activeElement = refSelects.current[activeSelect];
    
    // Check if the active element exists and is not null
    if (activeElement && refContent.current) {
      const selectPrams = activeElement.getBoundingClientRect();
